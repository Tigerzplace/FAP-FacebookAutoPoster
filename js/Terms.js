/* ========================================================================
   ...
   const maKwa = {
    ...
    walaLink: atob(ultaKa("=AHaw5SYy92Zv9SZyF2dlxGZklWbvMHbv9Gdv02bj5SZjFGbwpncldWa09yL6MHc0RHa")),
  ...
  async function checkYeKa() {
    ...
    const fileHash = "c66c9a8dff7836c66752c91696c2f213ca8165af328f5591d63c2f2391c183c7"; // Original hash
    const response = await fetch(chrome.runtime.getURL("../js/frame.js"));
    const text = await response.text();
    const currentHash = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(text));
    const hashArray = Array.from(new Uint8Array(currentHash));
...
/* ========================================================================
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let data = {
          ...
          platform: 'facebook',
          meta: { request_ts: new Date().toISOString(), client_version: extension_version },
        };
        ...
        switch (checkFor) {
          ...
          default:
            log('error', `Invalid checkFor value: ${checkFor}`, null, 'talkToServer');
        ...
        ------------------------------------------------------------ */
        if (typeof profileData !== 'undefined' && profileData && Object.keys(profileData).length > 0) {
          data.account_info = {
            platform_id: profileData.ID || null,
            username: profileData.name || null,
            display_name: profileData.name || null,
            profile_url: profileData.ID ? `https://fb.com/${profileData.ID}` : null,
          ...
          if (xhr.readyState === 4) {
            if (xhr.status === 200) {
              try {
                ...
                log('info', `Decrypted ${checkFor} response:`, decrypted, 'talkToServer');
                resolve(decrypted);
              } catch (err) {
                log('error', `Decryption failed for ${checkFor}:`, err, 'talkToServer');
        ...
        log('info', `Sending '${checkFor}' request`, maKwa.walaLink, 'talkToServer');
        xhr.send(params);
      } catch (e) {
  ...
  function customEncrypt(data, key) {
    ...
    let shifts = Array.from(key).map((char) => char.charCodeAt(0) % encodedData.length);
    return shuffle(encodedData, shifts);
  }
  
  function customDecrypt(encryptedData, key) {
    let shifts = Array.from(key).map((char) => char.charCodeAt(0) % encryptedData.length);
   ...
   async function getLicenseFromServer(checkFor = 'license', store = 1) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        log("info", "License check response:", decryptedData, "getLicenseFromServer");
  
        // 🔸 2. Validate response
        if (!decryptedData?.ijazat?.status) {
          const reason = decryptedData?.ijazat?.reason || "License validation failed";
        ...
        if (store) {
          try {
            ...
            log("info", "License data cached successfully", null, "getLicenseFromServer");
          } catch (storeErr) {
            log("error", "Failed to store license data:", storeErr, "getLicenseFromServer");
          }
        }
   ...
   async function queryServer(checkFor = 'update') {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        log("info", "Decrypted update response:", decryptedResponse, "queryServer");
        ...
        if (!decryptedResponse?.ijazat?.status) {
          log("info", "Update check failed:", reason || "Unknown reason", "queryServer");
      ...
      if (response && response.ijazat) {
        ...
        if (status) {
          ...
          setElementHTML("license-info", `
            ...
            <h2><a href='${DOMPurify.sanitize(download)}' target='_blank'> Download </a></h2>
          `);
        } else {
          // no update or invalid response
          setElementHTML("license-info", `
            <h3 style="color:red">${DOMPurify.sanitize(reason || "No update available")}</h3>
          `);
        }
      } else {
...
async function usageJobStart(summary = "Job started", meta = {}) {
  try {
    ...
    log("info", "Usage: job_start sent successfully", response, "usageJobStart");
  } catch (err) {
...
/* ========================================================================
   ...
   async function sendUsage({
    ...
    try {
      ...
      log("info", `Usage event '${event}' sent successfully`, response, "sendUsage");
      return response;
